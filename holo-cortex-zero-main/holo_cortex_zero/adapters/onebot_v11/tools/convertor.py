...
from holo_cortex_zero.adapters.onebot_v11.tools.onebot_util import get_user_group_card_name
from holo_cortex_zero.core.config import config
from holo_cortex_zero.core.logger import logger
from holo_cortex_zero.core.os_env import NAPCAT_TEMPFILE_DIR
...
def _onebot_seg_type(seg: Any) -> str:
    return str(getattr(seg, "type", None) or (seg.get("type") if isinstance(seg, dict) else "") or "")


def _onebot_seg_data(seg: Any) -> dict[str, Any]:
...
def _resolve_onebot_attachment_mode(
    *,
    adapter: BaseAdapter,
    db_chat_channel: DBChatChannel,
    sender_platform_userid: str,
    attachment_kind: str,
) -> tuple[str, str]:
    preview = preview_canonical_inbound_identity(
        adapter=adapter,
        raw_platform_userid=sender_platform_userid,
        raw_channel_id=db_chat_channel.channel_id,
        channel_type=db_chat_channel.chat_type,
    )
    return resolve_incoming_attachment_mode(
        adapter_key=adapter.key,
        chat_key=preview.canonical_chat_key,
        chat_type=db_chat_channel.chat_type.value if isinstance(db_chat_channel.chat_type, ChatType) else str(db_chat_channel.chat_type),
        sender_id=preview.canonical_userid,
        platform_userid=preview.canonical_userid,
        attachment_kind=attachment_kind,
        channel_type=db_chat_channel.chat_type.value if isinstance(db_chat_channel.chat_type, ChatType) else str(db_chat_channel.chat_type),
    )


def _preview_onebot_attachment_identity(
    *,
    adapter: BaseAdapter,
    db_chat_channel: DBChatChannel,
    sender_platform_userid: str,
...
async def _build_onebot_reference_segment(
    *,
    reply_id: str,
    bot: Bot,
    db_chat_channel: DBChatChannel,
    adapter: BaseAdapter,
) -> Optional[ChatMessageSegmentReference]:
    ...
    try:
        ...
        logger.info(f"OneBot 引用消息拉取失败: msg_id={reply_id} err={exc}")
        return build_reference_segment(
            ref_msg_id=reply_id,
            ref_chat_key=db_chat_channel.chat_key,
            ref_sender_id="",
            ref_sender_name="",
    ...
    if not isinstance(sender, dict):
        sender = {}
    ref_sender_id = str(sender.get("user_id") or (payload.get("user_id") if isinstance(payload, dict) else "") or "")
    ref_sender_name = str(sender.get("card") or sender.get("nickname") or ref_sender_id or "")
    ref_send_timestamp = int((payload.get("time") if isinstance(payload, dict) else 0) or 0)
    raw_message = payload.get("message") if isinstance(payload, dict) else None

    ref_segments: List[ChatMessageSegment] = []
    if isinstance(raw_message, str):
        if raw_message.strip():
            ref_segments.append(ChatMessageSegment(type=ChatMessageSegmentType.TEXT, text=raw_message))
    else:
        for item in raw_message or []:
            seg_type = _onebot_seg_type(item)
            seg_data = _onebot_seg_data(item)
            if seg_type == "reply":
                continue
            if seg_type == "text":
                ref_segments.append(ChatMessageSegment(type=ChatMessageSegmentType.TEXT, text=str(seg_data.get("text") or "")))
                continue
            if seg_type == "image":
                ref_chat_key, ref_effective_sender_id = _preview_onebot_attachment_identity(
                    adapter=adapter,
                    db_chat_channel=db_chat_channel,
                    sender_platform_userid=ref_sender_id,
                )
                image_mode, image_reason = _resolve_onebot_attachment_mode(
                    adapter=adapter,
                    db_chat_channel=db_chat_channel,
                    sender_platform_userid=ref_sender_id,
                ...
                logger.info(
                    f"OneBot 引用图片接收策略: chat_key={ref_chat_key} sender={ref_effective_sender_id} raw_sender={ref_sender_id} mode={image_mode} reason={image_reason}",
                )
                suffix = ""
                try:
                    if "filename" in seg_data:
                        suffix = "." + str(seg_data["filename"]).split(".")[-1].lower()
                    elif "file_id" in seg_data:
                        suffix = "." + str(seg_data["file_id"]).split(".")[-1].lower()
                    elif "file" in seg_data:
                        suffix = "." + str(seg_data["file"]).split(".")[-1].lower()
                except Exception:
                    suffix = ""
                try:
                    if "url" in seg_data:
                        ref_segments.append(
                            await ChatMessageSegmentImage.create_from_url(
                                url=str(seg_data["url"]),
                                from_chat_key=ref_chat_key,
                                use_suffix=suffix,
                                ingest_mode=image_mode,
                            ),
                        )
                    elif "file" in seg_data:
                        local_path = str(seg_data["file"])
                    ...
                    logger.info(f"OneBot 引用图片构建失败: msg_id={reply_id} err={exc}")
                continue
            if seg_type == "file":
                ref_chat_key, ref_effective_sender_id = _preview_onebot_attachment_identity(
                    adapter=adapter,
                ...
                file_mode, file_reason = _resolve_onebot_attachment_mode(
                    adapter=adapter,
                    db_chat_channel=db_chat_channel,
                    sender_platform_userid=ref_sender_id,
                ...
                logger.info(
                    f"OneBot 引用文件接收策略: chat_key={ref_chat_key} sender={ref_effective_sender_id} raw_sender={ref_sender_id} mode={file_mode} reason={file_reason}",
                )
                try:
                    if "url" in seg_data:
                        ref_segments.append(
                    ...
                    logger.info(f"OneBot 引用文件构建失败: msg_id={reply_id} err={exc}")
                continue
            if seg_type == "json":
                ref_segments.append(ChatMessageSegment(type=ChatMessageSegmentType.TEXT, text=JSON_CARD_FALLBACK_TEXT))

    return build_reference_segment(
        ref_msg_id=reply_id,
        ref_chat_key=db_chat_channel.chat_key,
        ref_sender_id=ref_sender_id,
        ref_sender_name=ref_sender_name,
        ref_send_timestamp=ref_send_timestamp,
...
async def convert_chat_message(
    ...
    is_tome = False
    sender_platform_userid = str(
        getattr(ob_event, "user_id", None)
        or getattr(getattr(ob_event, "sender", None), "user_id", None)
        or ""
    )

    if isinstance(ob_event, GroupUploadNoticeEvent):
        effective_chat_key, effective_sender_id = _preview_onebot_attachment_identity(
            adapter=adapter,
            db_chat_channel=db_chat_channel,
            sender_platform_userid=sender_platform_userid,
        )
        file_ingest_mode, file_reason = _resolve_onebot_attachment_mode(
            adapter=adapter,
            db_chat_channel=db_chat_channel,
            sender_platform_userid=sender_platform_userid,
            attachment_kind="file",
        ...
        logger.info(
            f"OneBot 附件接收策略: chat_key={effective_chat_key} sender={effective_sender_id} raw_sender={sender_platform_userid} mode={file_ingest_mode} reason={file_reason}",
        )
        ...
        if ob_event.file.model_extra and ob_event.file.model_extra.get("url"):
            ...
            # napcat 挂载目录 ${HCZ_DATA_DIR}/napcat_data/QQ:/app/.config/QQ
    ...
    for seg in ob_message:
        if seg.type == "text":
            ...
            reference_segment = await _build_onebot_reference_segment(
                reply_id=reply_id,
                bot=bot,
                db_chat_channel=db_chat_channel,
                adapter=adapter,
            ...
            if reference_segment:
                ret_list.append(reference_segment)
            ...
            image_ingest_mode, image_reason = _resolve_onebot_attachment_mode(
                adapter=adapter,
                db_chat_channel=db_chat_channel,
                sender_platform_userid=sender_platform_userid,
                attachment_kind="image",
            ...
            logger.info(
                f"OneBot 附件接收策略: chat_key={effective_chat_key} sender={effective_sender_id} raw_sender={sender_platform_userid} mode={image_ingest_mode} reason={image_reason}",
            )
            try:
            ...
            if at_qq == bot_qq:
                at_qq = bot_qq
                is_tome = True
                nick_name = db_chat_channel.get_persona_display_name()
            else:
    ...
    if msg_to_me and not is_tome:
        ...
        ret_list.insert(
            ...
            ChatMessageSegmentAt(
                ...
                target_nickname=db_chat_channel.get_persona_display_name(),
            ),
        )
