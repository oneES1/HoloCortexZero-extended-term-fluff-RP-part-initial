...
def patch_adapter_file(filepath: Path) -> int:
    ...
    while i < len(lines):
        ...
        if dict_match:
            ...
            # Convert existing dict items to kwargs
            extra_lines = [f"{indent}    {existing},"] if existing else []
            extra_lines.append(i18n_block)
            replacement = f"json_schema_extra=ExtraField(\n" + "\n".join(extra_lines) + f"\n{indent}).model_dump()"
