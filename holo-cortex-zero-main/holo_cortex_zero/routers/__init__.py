...
def _webui_static_file(static_dir: Path, filename: str) -> FileResponse:
...
def mount_api_routes(app: FastAPI):
    ...
    try:
        ...
        if static_dir.exists():
            ...
            # iOS/PWA 会探测同域根路径；这里显式固定为 HCZ 图标，避免同域 /code 的 code-server 图标污染。
            @app.api_route("/apple-touch-icon.png", methods=["GET", "HEAD"], include_in_schema=False)
            async def webui_apple_touch_icon():
                return _webui_static_file(static_dir, "apple-touch-icon.png")

            @app.api_route("/apple-touch-icon-precomposed.png", methods=["GET", "HEAD"], include_in_schema=False)
            async def webui_apple_touch_icon_precomposed():
                return _webui_static_file(static_dir, "apple-touch-icon.png")

            @app.api_route("/favicon.ico", methods=["GET", "HEAD"], include_in_schema=False)
            async def webui_favicon_ico():
                return _webui_static_file(static_dir, "favicon-dark-512.png")

            @app.api_route("/favicon.png", methods=["GET", "HEAD"], include_in_schema=False)
            async def webui_favicon_png():
                return _webui_static_file(static_dir, "favicon-dark-512.png")

            @app.api_route("/manifest.json", methods=["GET", "HEAD"], include_in_schema=False)
            async def webui_manifest():
                return _webui_static_file(static_dir, "manifest.json")
