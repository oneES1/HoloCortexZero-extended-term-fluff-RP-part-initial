...
def _format_datetime(dt_str: Any) -> str:
    """Parse datetime string and convert to local system time (UTC+8)."""
    if not dt_str or dt_str == "-":
        return "-"
    try:
        # Parse the datetime string (handles ISO 8601 etc)
        if isinstance(dt_str, str):
            dt = parser.parse(dt_str)
        else:
            # If it's already a datetime object (though type hint is Any)
            dt = dt_str
        ...
        # 转换为 UTC+8 (Beijing Time)
        tz_cn = timezone(timedelta(hours=8))
        dt_local = dt.astimezone(tz_cn)
...
def _fmt_entry_common(idx: int, item: Mapping[str, Any]) -> List[str]:
    ...
    created = _format_datetime(_safe_get(item, "created_at", "-"))
    updated = _format_datetime(_safe_get(item, "updated_at", "-"))
...
def _single_line_text(v: Any, max_len: int = 240) -> str:
    try:
        s = str(v or "").replace("\n", " ").strip()
        s = " ".join(s.split())
...
def _format_datetime_prompt(dt_str: Any) -> str:
    """Prompt 注入用：将时间转换为系统本地时区并压缩格式。"""
    if not dt_str or dt_str == "-":
        return ""
    try:
        if isinstance(dt_str, str):
        ...
        try:
            return _single_line_text(dt_str, max_len=32)
        except Exception:
            return ""


def format_prompt_grouped_memories(items: List[Dict[str, Any]], *, max_items_per_user: int = 16) -> List[str]:
    """给主模型 Prompt 注入用的精简版记忆格式。

    目标：减少 token/噪音，不注入记忆 ID、TYPE/标签、相关度等细节；但保留时间节点与置信度。
    """
    if not items:
        return []
    ...
    for user_id, user_items in grouped.items():
        lines: List[str] = [f"用户: {user_id}（{len(user_items)} 条）"]
        for item in user_items[: max(0, int(max_items_per_user or 0))]:
            memory = _single_line_text(_safe_get(item, "memory", ""))
            if memory:
                ts = _safe_get(item, "updated_at") or _safe_get(item, "created_at")
                ts_s = _format_datetime_prompt(ts)

                confidence = ""
                md = _safe_get(item, "metadata", {})
                ...
                lines.append(f"- {prefix}{memory}" if prefix else f"- {memory}")
        if len(lines) > 1:
            parts.append("\n".join(lines))
