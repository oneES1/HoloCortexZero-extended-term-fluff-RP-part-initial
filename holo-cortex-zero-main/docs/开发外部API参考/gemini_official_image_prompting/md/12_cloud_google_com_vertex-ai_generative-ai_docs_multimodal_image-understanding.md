...
> Source: https://docs.cloud.google.com/vertex-ai/generative-ai/docs/multimodal/image-understanding
...
[![](https://docs.cloud.google.com/static/vertex-ai/images/colab-logo-32px.png)Open in Colab](https://colab.research.google.com/github/GoogleCloudPlatform/generative-ai/blob/main/gemini/use-cases/intro_multimodal_use_cases.ipynb)
|
...
in Vertex AI Workbench](https://console.cloud.google.com/vertex-ai/workbench/deploy-notebook?download_url=https%3A%2F%2Fraw.githubusercontent.com%2FGoogleCloudPlatform%2Fgenerative-ai%2Fmain%2Fgemini%2Fuse-cases%2Fintro_multimodal_use_cases.ipynb)
|
[![](https://docs.cloud.google.com/static/vertex-ai/images/github-logo-32px.png)View on GitHub](https://github.com/GoogleCloudPlatform/generative-ai/blob/main/gemini/use-cases/intro_multimodal_use_cases.ipynb)
...
| --- | --- | --- |
| Gemini 3 Pro Image preview | Maximum images per prompt: 14 Maximum file size per file for inline data or direct uploads through the console: 7 MB Maximum file size per file from Google Cloud Storage: 30 MB Maximum number of output images per prompt: Limited to 32,768 output tokens Supported aspect ratios: 1:1, 3:2, 2:3, 3:4, 4:3, 4:5, 5:4, 9:16, 16:9, and 21:9 | image/png image/jpeg image/webp image/heic image/heif |
| Gemini 3 Pro preview Gemini 3 Flash preview | Maximum images per prompt: 900 Maximum file size per file for inline data or direct uploads through the console: 7 MB Maximum file size per file from Google Cloud Storage: 30 MB Default resolution tokens: 1120 | image/png image/jpeg image/webp image/heic image/heif |
| Gemini 2.5 Pro Gemini 2.5 Flash with Live API native audio (Preview) preview Gemini 2.5 Flash with Gemini Live API native audio Gemini 2.5 Flash preview Gemini 2.5 Flash | Maximum images per prompt: 3,000 Maximum file size per file for inline data or direct uploads through the console: 7 MB Maximum file size per file from Google Cloud Storage: 30 MB | image/png image/jpeg image/webp image/heic image/heif |
| Gemini 2.5 Flash-Lite preview Gemini 2.5 Flash-Lite | Maximum images per prompt: 3,000 Maximum file size per file for inline data or direct uploads through the console: 7 MB Maximum file size per file from Google Cloud Storage: 30 MB Maximum number of output images per prompt: 10 | image/png image/jpeg image/webp image/heic image/heif |
| Gemini 2.5 Flash Image | Maximum images per prompt: 3 Maximum file size per file for inline data or direct uploads through the console: 7 MB Maximum file size per file from Google Cloud Storage: 30 MB Maximum number of output images per prompt: 10 Supported aspect ratios: 1:1, 3:2, 2:3, 3:4, 4:3, 4:5, 5:4, 9:16, 16:9, and 21:9 | image/png image/jpeg image/webp image/heic image/heif |
| Gemini 2.0 Flash-Lite | Maximum images per prompt: 3,000 Maximum file size per file for inline data or direct uploads through the console: 7 MB Maximum file size per file from Google Cloud Storage: 30 MB Maximum tokens per minute (TPM): High/Medium/Default media resolution: US/Asia: 6.7 M EU: 2.6 M Low media resolution: US/Asia: 2.6 M EU: 2.6 M | image/png image/jpeg image/webp image/heic image/heif |
| Gemini 2.0 Flash | Maximum images per prompt: 3,000 Maximum file size per file for inline data or direct uploads through the console: 7 MB Maximum file size per file from Google Cloud Storage: 30 MB Maximum tokens per minute (TPM) per project: High/Medium/Default media resolution: US/Asia: 40 M EU: 10 M Low media resolution: US/Asia: 10 M EU: 2.6 M | image/png image/jpeg image/webp image/heic image/heif |

For a list of languages supported by Gemini models, see model information
[Google models](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/learn/models). To learn
...
5. Click **Insert Media**, and select a source for your file.
   ...
   3. Click **Select**.
      ...
      [token limit](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/learn/models#gemini-models), the
...
After you
[set up your environment](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/start/quickstarts/quickstart-multimodal#gemini-setup-environment-drest),
...
```
curl -X POST \     -H "Authorization: Bearer $(gcloud auth print-access-token)" \     -H "Content-Type: application/json; charset=utf-8" \     -d @request.json \     "https://aiplatform.googleapis.com/v1/projects/PROJECT_ID/locations/global/publishers/google/models/gemini-1.5-flash:generateContent"
```
...
```
$cred = gcloud auth print-access-token$headers = @{ "Authorization" = "Bearer $cred" }Invoke-WebRequest `    -Method POST `    -Headers $headers `    -ContentType: "application/json; charset=utf-8" `    -InFile request.json `    -Uri "https://aiplatform.googleapis.com/v1/projects/PROJECT_ID/locations/global/publishers/google/models/gemini-1.5-flash:generateContent" | Select-Object -Expand Content
```
...
- `LOCATION`: The region to process the
  ...
  [Available locations](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/learn/locations#available-regions).
...
```
  ...
  The [base64 encoding](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/image/base64-encode) of the image, PDF, or video
...
- Use the
  [`generateContent`](https://docs.cloud.google.com/vertex-ai/docs/reference/rest/v1/projects.locations.publishers.models/generateContent)
  ...
  [`streamGenerateContent`](https://docs.cloud.google.com/vertex-ai/docs/reference/rest/v1/projects.locations.publishers.models/streamGenerateContent)
  method.
...
```
curl -X POST \     -H "Authorization: Bearer $(gcloud auth print-access-token)" \     -H "Content-Type: application/json; charset=utf-8" \     -d @request.json \     "https://aiplatform.googleapis.com/v1/projects/PROJECT_ID/locations/global/publishers/google/models/gemini-2.5-flash:generateContent"
```
...
```
$cred = gcloud auth print-access-token$headers = @{ "Authorization" = "Bearer $cred" }Invoke-WebRequest `    -Method POST `    -Headers $headers `    -ContentType: "application/json; charset=utf-8" `    -InFile request.json `    -Uri "https://aiplatform.googleapis.com/v1/projects/PROJECT_ID/locations/global/publishers/google/models/gemini-2.5-flash:generateContent" | Select-Object -Expand Content
```
...
information, see [Content generation parameters](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/multimodal/content-generation-parameters).
...
- Learn how to [send chat prompt requests](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/multimodal/send-chat-prompts-gemini).
