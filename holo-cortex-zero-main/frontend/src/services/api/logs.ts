...
export const logsApi = {
  getLogs: async (params: { page: number; pageSize: number; source?: string; levels?: string[] }) => {
    const response = await axios.get<{ data: LogsResponse }>('/logs', {
      params: {
        ...params,
  ...
  downloadLogs: async (params: { lines?: number; source?: string }) => {
    try {
      ...
      let filename = 'holo_cortex_zero_logs.txt'
