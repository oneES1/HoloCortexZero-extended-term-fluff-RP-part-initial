...
async def get_chat_channel_list(
    ...
    统计逻辑：
    - 消息数量：仅统计conversation_start_time之后的消息
    - 最后消息时间：从整个消息表中查询
    """
    query = DBChatChannel
...
@require_platform_role(Role.Admin)
async def get_chat_channel_detail(chat_key: str, _platform_admin: PlatformAdminPrincipal = Depends(get_current_active_platform_admin)) -> Ret:
    """获取聊天频道详情"""
    ...
    return Ret.success(
        ...
        data={
            ...
            "is_active": channel.is_active,
            "chat_type": channel.chat_type.value,
            "persona_display_name": channel.get_persona_display_name(),
            "message_count": message_count,
