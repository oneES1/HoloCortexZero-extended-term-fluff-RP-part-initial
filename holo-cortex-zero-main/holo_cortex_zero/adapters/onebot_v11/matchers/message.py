...
from holo_cortex_zero.adapters.onebot_v11.tools.convertor import convert_chat_message
from holo_cortex_zero.adapters.onebot_v11.tools.onebot_util import (
    gen_chat_text,
    get_chat_info,
...
def register_matcher(adapter: BaseAdapter):
    ...
    async def _(_: Matcher, event: Union[MessageEvent, GroupMessageEvent], bot: Bot):
        ...
        content_text, is_tome = await gen_chat_text(
            event=event,
            bot=bot,
            db_chat_channel=db_chat_channel,
            runtime_cache=runtime_cache,
        ...
        ref_segment = extract_primary_reference_segment(content_data)
        ...
        plt_msg: PlatformMessage = PlatformMessage(
            ...
            ext_data=PlatformMessageExt(
                ref_chat_key=ref_segment.ref_chat_key if ref_segment else db_chat_channel.chat_key,
    ...
    async def _(_: Matcher, event: GroupUploadNoticeEvent, bot: Bot):
        ...
        identity_preview = preview_canonical_inbound_identity(
            adapter=adapter,
            raw_platform_userid=platform_userid,
            raw_channel_id=channel_id,
            channel_type=chat_type,
        )
        effective_platform_userid = identity_preview.canonical_userid
        user: Optional[DBUser] = await DBUser.get_or_none(
            adapter_key=adapter.key,
            platform_userid=effective_platform_userid,
        )

        if not user:
            if platform_userid == (await adapter.get_self_info()).user_id:
                return
            raise ValueError(f"用户 {effective_platform_userid} 尚未注册，请先发送任意消息注册后即可上传文件") from None

        if not user.is_active:
            logger.info(f"用户 {effective_platform_userid} 被封禁，封禁结束时间: {user.ban_until}")
            return
