...
$DistroInstallLocation = Join-Path $env:LOCALAPPDATA "HoloCortexZero"
...
Write-Host "HoloCortexZero WSL 安装脚本"
...
$DistroName = "holo-cortex-zero"
$WorkDir = (Join-Path $env:TEMP ("HoloCortexZero_" + (Get-Date -Format "yyyyMMddHH") + (Get-Random)))
...
Write-Warning "HoloCortexZero 将安装在 '$DistroInstallLocation'"
...
if ($WSLStatus.Version -ne "2") {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "HoloCortexZero 的使用依赖于 WSL2，可能导致后续 Docker 功能无法使用" -ForegroundColor RED
        Write-Host "请尝试执行 'wsl.exe --set-default-version 2' 以手动设置" -ForegroundColor RED
        Write-Host "或使用 'wsl --set-version $DistroName 2' 单独为 HoloCortexZero 设置"
    }
}
...
$installScriptPath = Join-Path $PSScriptRoot "install.sh"
if ($installScriptPath -match '^(?<drive>[A-Za-z]):\\(?<rest>.*)$') {
    $installScriptPath = "/mnt/$($Matches.drive.ToLower())/" + ($Matches.rest -replace '\\', '/')
}

$installNACommand = @'
#!/bin/bash

INSTALL_SCRIPT='__HCZ_INSTALL_SCRIPT__'
if [ ! -f "$INSTALL_SCRIPT" ]; then
    echo '未找到本地 install.sh，请从仓库 docker/ 目录运行该脚本' >&2
    exit 1
fi
echo "正在调用本地安装脚本: $INSTALL_SCRIPT"
echo '用户 hcz 密码为 123456'
sudo bash "$INSTALL_SCRIPT"
'@
$installNACommand = $installNACommand.Replace('__HCZ_INSTALL_SCRIPT__', $installScriptPath)
if ($InstallStatus.DepsInstalled) {
    Write-Host "正在安装 HoloCortexZero..."
    Invoke-WslCommand -DistributionName $DistroName -Username $newUserName -Command $installNACommand
    if ($LASTEXITCODE -eq 0) {
        Write-Host "HoloCortexZero 安装成功"  -ForegroundColor Green
        $InstallStatus.NAInstalled = $true
    } else {
        Write-Host "HoloCortexZero 安装失败" -ForegroundColor Red
    }
} else {}

Write-Host ("-"*40)
if ($InstallStatus.NAInstalled) {
    Write-Host "HoloCortexZero 已安装成功！"
} else {
    if (-not $InstallStatus.DepsInstalled) {
        Write-Host "依赖未安装：curl"
    }
    Write-Host "HoloCortexZero 安装失败"
    Write-Host "请前往 WSL 实例 `"$DistroName`" 手动完成安装"
    Write-Host "请从仓库 docker/ 目录再次运行安装器，或手动执行同目录的 install.sh"
    Write-Host "若仍失败，请参考文档站 WSL2 部署教程"
}
Write-Host "可使用 'wsl -d `"$DistroName`"' 进入 HoloCortexZero 所在 WSL 实例"
